...
def main():
    ...
    data_config = KFoldConfig(k=10, split=split, data_type="f", art_rejection=True, filtering=True, resample=None,
    ...
    kfoldcv = KFold_CV(data_config, optimiser_config, device, MLP1)
    timer = Timer()
    timer.start()
    kfoldcv.cross_validate(n_epochs=12, print_losses=True, weight_losses=True, weight_scalings=torch.tensor([1, 1, 1, 1]))
