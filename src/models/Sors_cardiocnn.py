...
# It is designed to take feature length 7500 which corresponds to one 30s epoch at 125Hz, i.e. without surrounding context.
# The first 3750 samples should be ecg, the next 3750 should be rip. The CNN takes two input channels to deal with these two data types.
class Sors_cardiocnn(nn.Module):

    def __init__(self):
        super(Sors_cardiocnn, self).__init__()
        ...
        self.conv1 = nn.Conv1d(2, 128, 7, 2, padding=padding7)
    ...
    def forward(self, x):
        ...
        x = torch.cat((ecg, rip), dim=1)  # Concatenated along channel dimension
