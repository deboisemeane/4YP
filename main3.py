...
def main():
    ...
    data_config = KFoldConfig(k=10, split=split, data_type="t", art_rejection=True, filtering=True, resample=None,
    ...
    timer = Timer()
    timer.start()
    kfoldcv.cross_validate(n_epochs=12, print_losses=True, weight_losses=False, weight_scalings=None)
    time_train = timer.stop()
    print(f"Total training time: {time_train}")

    # Plotting loss for training with SHHS
    kfoldcv.plot_loss("/figures/5fold_Sors_EEG.png", "5-Fold Cross Validation of Model 2")
